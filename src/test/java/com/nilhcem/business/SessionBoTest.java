...
public class SessionBoTest extends AbstractDbTest {
	...
	@Before
	public void setUp() {
		fiveDaysago = calendar.getCustomDateFromToday(-5);
	}
	...
	public void testFillSessionWithData() {
		...
		createNoteWithDuDate(user, calendar.getCustomDateFromToday(-3));
		MockHttpSession session = new MockHttpSession();
		sessionBo.fillSession(false, session);
		...
		createNoteWithDuDate(user, calendar.getDateToday());
		...
		createNoteWithDuDate(user, calendar.getDateTomorrow());
		session = new MockHttpSession();
		sessionBo.fillSession(false, session);
		assertEquals(2L, session.getAttribute("today"));
		...
		createNoteWithDuDate(user, calendar.getDateNextWeek());
		session = new MockHttpSession();
		sessionBo.fillSession(false, session);
		assertEquals(2L, session.getAttribute("today"));
		assertEquals(1L, session.getAttribute("tomorrow"));
		...
		createNoteWithDuDate(user, calendar.getCustomDateFromToday(8));
		session = new MockHttpSession();
		sessionBo.fillSession(false, session);
		assertEquals(2L, session.getAttribute("today"));
		assertEquals(1L, session.getAttribute("tomorrow"));
